  ...
  <div class="truncated-text-display">
    ...
    <div class="content-actions">
      <div class="action-buttons">
        <button v-if="isTruncated" @click="toggleShowAllContent" class="toggle-button">
          {{ displayMode === 'showAll' ? 'Show Less' : 'Show All' }}
        </button>

        <button v-if="isTruncated && displayMode !== 'showAll'" @click="toggleCollapseFirstLast" class="toggle-button">
          {{ displayMode === 'collapseFirstLast' ? 'Expand' : 'Collapse' }}
        </button>
      </div>

      <div class="pagination-controls" v-if="isTruncated && displayMode === 'paginate'">
        <button @click="prevPage" class="pagination-button" :disabled="currentPage === 0">
        ...
        <button @click="nextPage" class="pagination-button" :disabled="currentPage === totalPages - 1">
          <svg xmlns="http://www.w3.org/2000/svg" width="18" height="18" viewBox="0 0 24 24" fill="none"
            stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round">
      ...
      <div class="copy-container">
        <span class="copy-icon" @click="copyToClipboard" :class="{ copied: isCopied }" aria-label="Copy to clipboard"
